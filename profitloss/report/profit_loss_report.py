...
class profitloss_report(osv.osv):
    ...
    _columns = {
        ...
        'inv_balance': fields.float('Amount', readonly=True),
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,
        ...
        for a in [['period_id','in','current_year'], ['period_id','in','current_period']]:
            if a in args:
                args.remove(a)
        return super(profitloss_report, self).search(cr, uid, args=args, offset=offset, limit=limit, order=order,
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False,lazy=True):
        ...
        if context.get('period', False) == 'current_period':
            ...
            domain = domain
        return super(profitloss_report, self).read_group(cr, uid, domain, fields, groupby, offset, limit, context, orderby,lazy)

    def init(self, cr):
        tools.drop_view_if_exists(cr, 'profitloss_report')
        cr.execute("""
            create or replace view profitloss_report as (
            select
                l.id as id,
                am.date as date,
                ...
                -1 * (coalesce(l.debit, 0.0) - coalesce(l.credit, 0.0)) as inv_balance
            from
                account_move_line l
